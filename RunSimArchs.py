...
def prepare_sequences(seq_len):
    x = []
    with gzip.open("data/simdata_%s.sequences.gz" % str(seq_len), "r") as f:
        for line in f:
            curr = line[:seq_len].decode("utf-8")
            x.append(curr)
    embeddings1 = read_simdata_file(
        "data/DensityEmbedding_motifs-ELF1_known2_min-1_max-3_mean-2_zeroProb-0_seqLength-%s_numSeqs-10000.simdata" % str(
            seq_len)).embeddings
    embeddings2 = read_simdata_file(
        "data/DensityEmbedding_motifs-GATA_known6_min-1_max-3_mean-2_zeroProb-0_seqLength-%s_numSeqs-10000.simdata" % str(
            seq_len)).embeddings
    embeddings3 = read_simdata_file(
        "data/DensityEmbedding_motifs-RXRA_known1_min-1_max-3_mean-2_zeroProb-0_seqLength-%s_numSeqs-10000.simdata" % str(
            seq_len)).embeddings
